...
if ($koneksipusat) {
    ...
    if ($query['status'] == 'aktif') {
        ...
        if ($data == 'DATA3') {
            ...
            while ($r = mysqli_fetch_array($sqlcek)) {
                $sql = mysqli_query($koneksi, "insert into soal
						...
						('$r[id_soal]','$r[id_mapel]','$r[nomor]','$r[soal]','$r[jenis]','$r[pilA]','$r[pilB]','$r[pilC]','$r[pilD]','$r[pilE]','$r[jawaban]','$r[file]','$r[file1]','$r[fileA]','$r[fileB]','$r[fileC]','$r[fileD]','$r[fileE]')");
                if (!$sql) {
                    $gagal3++;
                } else {
                    $masuk3++;
        ...
        if ($data == 'DATA5') {
            ...
            $sql = mysqli_query($koneksi, "UPDATE setting SET sekolah='$r[sekolah]',
