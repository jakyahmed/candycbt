    ...
    function import_ques($question)
    {
        ...
        foreach ($question as $key => $singlequestion) {
            ...
            if ($key != 0) {
                ...
                $ques_type = "1";
                if ($option_count != "0") { //ada pilihan ganda
                    if ($singlequestion['correct'] != "") { //ada kunci
                        $ques_type = "1"; //soal pil ganda
                    } else {
                        //tidak ada kunci jawaban
                        echo "Ada soal yang tidak memiliki kunci jawaban, perbaiki.<br>";
                        echo "Nomor soal :".($no+1)."<br>";
                        echo  $singlequestion['question'];
                        exit;
                    }
                } else { //tidak ada pilihan ganda
                    if ($singlequestion['correct'] != "") { //jika ada kunci
                        $ques_type = "1"; // soal pil ganda
                    } else { //tidak ada kunci
                        $ques_type = "2"; //soal esai
                    }
                }

                $kunci = strtoupper(trim($singlequestion['correct']));
                ...
                if ($this->db->insert('soal', $insert_data)) {
                    ...
                    if ($ques_type == "1") {
                        if (!empty($singlequestion['option'])) {
                            $update_data = array();
                ...
                //insert soal gagal
                    ...
                    echo "<pre> Gagal mengimport soal.</pre>";
                    exit;
                }
            } //key==0
